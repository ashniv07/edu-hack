...
import { parsePythonTraceback } from './diagnostics';
import { getOutputChannel } from './outputChannel';
import { executeProcess } from './process';
import type { FileInfo, RuntimeResult } from './types';

interface RunPythonFileOptions {
	fileInfo: FileInfo;
	outputChannel?: vscode.OutputChannel;
}

export async function runPythonFile(options: RunPythonFileOptions): Promise<RuntimeResult> {
